...
namespace gazebo
{
  ...
  private:
	...
	public: 
		...
		void ctrl_loop();
		void traj_cb( geometry_msgs::Point );
};
...
ClikPlugin::ClikPlugin(){
	...
	_destination_sub = _nh->subscribe("/destination", 0, &ClikPlugin::traj_cb, this);
	...
	timeseries.resize((delta*finaltime)+1);
...
void ClikPlugin::traj_cb(geometry_msgs::Point dest){
	...
	l_x = dest.x - _p_out.p.x();
	l_y = dest.y - _p_out.p.y();
	l_z = dest.z - _p_out.p.z();
	...
	for(int i=1;i<((finaltime*delta)+1);i++){

		timeseries[i].p.data[0] = timeseries[i-1].p.data[0] + (l_x/(finaltime*delta));
		timeseries[i].p.data[1] = timeseries[i-1].p.data[1] + (l_y/(finaltime*delta));
		timeseries[i].p.data[2] = timeseries[i-1].p.data[2] + (l_z/(finaltime*delta));
